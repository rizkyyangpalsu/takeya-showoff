...
class RecordPrepaidExpenses
{
    ...
    public function handle(Departure $departure, Departure\Allowance $allowance, Staff $staff, array $additionalData = []): void
    {
        ...
        /** @var Journal $journal */
        $journal = $ledger->credit($cash, $allowance->amount, $staff)
            ->debit($prepaidExpense, $allowance->amount, $staff)
            ->record($allowance, $additionalData['memo'] ?? null, $additionalData['ref'] ?? null, $allowance->office->id);
